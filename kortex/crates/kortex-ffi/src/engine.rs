...
use kortex_vector::quant::RaBitQ;
...
/// via a future FFI extension, but v0's `kx_search` has no such parameter.
...
/// the integer status codes documented in `kortex.h`.
#[derive(Debug)]
...
impl Engine {
    ...
    ///
    /// Mirrors `kortex_vector::search::two_stage_search(&rabitq, &floatstore,
...
mod tests {
    ...
    /// (the same pattern `kortex-harness::run_scale_bench` uses).
