...
//! `extern "C"` FFI path (`sk_open`/`sk_search`, called directly from Rust —
...
use skinki_ffi::ffi::{sk_engine, sk_free_engine, sk_open, sk_search, SK_OK};
...
/// Mirrors `skinki-harness::run_scale_bench`'s flat-index build: stream
...
fn ffi_search_matches_two_stage_search() {
    ...
    let seed = 2026;
    let k = 8;
    let refine = skinki_ffi::engine::DEFAULT_REFINE;

    let dir = std::env::temp_dir().join(format!("skinki_ffi_parity_{}", std::process::id()));
    ...
    assert_eq!(rc, SK_OK, "sk_open failed");
    ...
    assert_eq!(rc, SK_OK, "sk_search failed");
