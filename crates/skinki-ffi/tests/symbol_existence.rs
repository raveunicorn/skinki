...
//! `include/skinki.h` exactly matches the exported symbols").
//!
//! This crate's `rlib`/`cdylib`/`staticlib` are all built from the same
//! `src/ffi.rs`, so the simplest deterministic check is: parse every `sk_*(`
//! function name out of the header, and assert each one is reachable as a
//! public item of `skinki_ffi::ffi` (i.e. the symbol the linker will export
...
/// Every symbol declared in `include/skinki.h`. Kept in sync by hand; the
...
/// Extract `sk_<name>(` identifiers from the header text (a tiny hand-rolled
...
fn symbols_in_header(text: &str) -> Vec<String> {
    ...
    while i < bytes.len() {
        if bytes[i..].starts_with(b"sk_") {
            ...
            // `sk_engine` typedef and the SK_ERR_* macro names (which don't
            // start with lowercase `sk_`).
            let mut k = j;
...
fn header_symbols_match_expected_list() {
    let header_path = Path::new(env!("CARGO_MANIFEST_DIR")).join("include/skinki.h");
    let text = std::fs::read_to_string(&header_path).expect("read skinki.h");
    ...
    assert_eq!(
        ...
        "skinki.h declared symbols != EXPECTED_SYMBOLS (update one or the other)"
    );
}

/// Assert each expected symbol is a real, callable item exported by
/// `skinki_ffi::ffi` — i.e. it resolves at compile time, which for
...
fn expected_symbols_are_exported_from_ffi_module() {
    use skinki_ffi::ffi::{sk_free_engine, sk_last_error, sk_open, sk_search, sk_version};
    ...
    let ptrs: [usize; 5] = [
        sk_open as *const () as usize,
        sk_search as *const () as usize,
        sk_free_engine as *const () as usize,
        sk_last_error as *const () as usize,
        sk_version as *const () as usize,
    ];
