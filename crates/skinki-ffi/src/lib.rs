...
//! `skinki_vector::search::two_stage_search` path on the same seeded index +
//! query. The public C surface is declared in `include/skinki.h` and
//! implemented in [`ffi`].
//!
//! ## Module layout
...
//! - [`error`] — thread-local last-error slot used by `sk_last_error`.
