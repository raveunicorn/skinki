//! The C-ABI boundary (`skinki.h`). **All `unsafe` in this crate lives here.**
//!
//! Design recap (locked in `specs/STAGE_6.md` section 3):
//! - `sk_engine*` is an opaque handle; callers never see Rust types.
//! - `sk_search` writes into a **caller-allocated** `out_ids[k]` buffer and
...
//!   `sk_engine*`, which `sk_free_engine` destroys."
//! - Status codes: `0` = OK, negative = error; `sk_last_error()` returns a
...
//! `SK_ERR_PANIC`. This is the "wrap every body" option from the spec (the
...
//!
//! 1. `sk_open`: dereferences `out_engine` (write) and reads `index_dir` as a
//!    C string. Both are raw-pointer reads/writes whose validity is the
//!    caller's contract per `skinki.h`; we null-check before use and bail out
//!    with an error code (not UB) if either is null.
//! 2. `sk_search`: dereferences `engine` (read-only borrow of the boxed
...
//! 3. `sk_free_engine`: reconstructs the `Box<Engine>` via `Box::from_raw` and
//!    drops it. Sound iff `engine` is either null (no-op) or a pointer
//!    previously returned by `sk_open` and not yet freed — this is the
//!    standard "one `Box::into_raw`/`Box::from_raw` pair" pattern; `sk_open` is
//!    the only function that produces these pointers (via `Box::into_raw`).
//! 4. `sk_last_error` / `sk_version`: build a `CString` (safe) and hand out its
//!    raw pointer. The `CString` for `sk_last_error` is owned by thread-local
...
//!    contract. `sk_version`'s `CString` is `'static` (leaked once via
...
/// Opaque handle type named in `skinki.h` (`typedef struct sk_engine
/// sk_engine`). The C side only ever sees `sk_engine*`; the real type is
...
pub struct sk_engine {
    inner: Engine,
}

// Status codes — keep in sync with the comments in `include/skinki.h`.
pub const SK_OK: i32 = 0;
pub const SK_ERR_NULL_PTR: i32 = -1;
pub const SK_ERR_INVALID_UTF8: i32 = -2;
pub const SK_ERR_OPEN_FAILED: i32 = -3;
pub const SK_ERR_SEARCH_FAILED: i32 = -4;
pub const SK_ERR_PANIC: i32 = -5;

/// Turn a caught panic payload into a human-readable string for
/// `sk_last_error`. Panic payloads are typically `&str` or `String`.
fn panic_message(payload: Box<dyn std::any::Any + Send>) -> String {
    if let Some(s) = payload.downcast_ref::<&str>() {
        ...
        "skinki-ffi: panic with non-string payload".to_string()
    }
}
...
/// [`sk_free_engine`]. Returns `SK_OK` (0) on success, or a negative
/// `SK_ERR_*` code; on error, `sk_last_error()` describes the failure and
...
/// writable `sk_engine**`. Both are caller-supplied per the `skinki.h`
/// contract; see the module-level "unsafe inventory" item 1.
#[no_mangle]
pub unsafe extern "C" fn sk_open(index_dir: *const c_char, out_engine: *mut *mut sk_engine) -> i32 {
    let result = std::panic::catch_unwind(|| {
        clear_last_error();

        if index_dir.is_null() || out_engine.is_null() {
            set_last_error("sk_open: index_dir or out_engine is null");
            return SK_ERR_NULL_PTR;
        }
        ...
        let path_str = match c_str.to_str() {
            Ok(s) => s,
            Err(_) => {
                set_last_error("sk_open: index_dir is not valid UTF-8");
                return SK_ERR_INVALID_UTF8;
            }
        };

        match Engine::open(Path::new(path_str)) {
            Ok(engine) => {
                let boxed = Box::new(sk_engine { inner: engine });
                // SAFETY: `out_engine` is a non-null, writable `sk_engine**`
                // per the function contract. `Box::into_raw` hands ownership
                // of the boxed `Engine` to the caller; it is reclaimed exactly
                // once, by `sk_free_engine`, via the matching `Box::from_raw`.
                unsafe {
            ...
            Err(e) => {
                set_last_error(format!("sk_open: {e}"));
                SK_ERR_OPEN_FAILED
            }
        }
    });
    ...
    result.unwrap_or_else(|payload| {
        set_last_error(format!("sk_open: panic: {}", panic_message(payload)));
        SK_ERR_PANIC
    })
}
...
/// ids actually written into `*out_len`. Returns `SK_OK` (0) on success or a
/// negative `SK_ERR_*` code; `sk_last_error()` describes any failure.
///
/// # Safety
/// `engine` must be a live pointer returned by [`sk_open`] and not yet passed
/// to [`sk_free_engine`]. `query` must point to `dim` readable `f32`s.
...
/// item 2.
#[no_mangle]
pub unsafe extern "C" fn sk_search(
    engine: *const sk_engine,
    query: *const f32,
    dim: usize,
    k: usize,
    out_ids: *mut u32,
    ...
    let result = std::panic::catch_unwind(|| {
        ...
        if engine.is_null() || query.is_null() || out_ids.is_null() || out_len.is_null() {
            set_last_error("sk_search: a required pointer argument is null");
            return SK_ERR_NULL_PTR;
        }

        // SAFETY: `engine` is a live handle from `sk_open` per the contract;
        ...
        match engine.inner.search(query_slice, k, out_slice) {
            ...
            Err(e) => {
                set_last_error(format!("sk_search: {e}"));
                SK_ERR_SEARCH_FAILED
            }
        }
    });

    result.unwrap_or_else(|payload| {
        set_last_error(format!("sk_search: panic: {}", panic_message(payload)));
        SK_ERR_PANIC
    })
}

/// Free an engine handle previously returned by [`sk_open`]. `engine` may be
/// null (no-op). After this call, `engine` must not be used again.
///
/// # Safety
/// `engine` must be either null or a pointer previously returned by
/// [`sk_open`] that has not already been freed. See module-level "unsafe
/// inventory" item 3.
#[no_mangle]
pub unsafe extern "C" fn sk_free_engine(engine: *mut sk_engine) {
    ...
    let _ = std::panic::catch_unwind(|| {
        ...
        // previously produced by `Box::into_raw` in `sk_open` and not yet
...
/// the next `sk_*` call on this thread (or thread exit); callers must not free
/// it.
///
/// # Safety
...
mod tests {
    ...
    fn temp_dir(tag: &str) -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("skinki_ffi_abi_{tag}_{}", std::process::id()));
    ...
    fn version_is_nonempty() {
        let v = unsafe { CStr::from_ptr(sk_version()) };
    ...
    fn open_search_free_roundtrip() {
        ...
        let mut engine: *mut sk_engine = std::ptr::null_mut();
        let rc = unsafe { sk_open(c_path.as_ptr(), &mut engine) };
        assert_eq!(rc, SK_OK);
    ...
    fn open_null_args_errors() {
        clear_last_error();
        let mut engine: *mut sk_engine = std::ptr::null_mut();
        let rc = unsafe { sk_open(std::ptr::null(), &mut engine) };
        assert_eq!(rc, SK_ERR_NULL_PTR);
        let msg = unsafe { CStr::from_ptr(sk_last_error()) };
    ...
    fn open_missing_dir_errors() {
        ...
        let mut engine: *mut sk_engine = std::ptr::null_mut();
        let rc = unsafe { sk_open(c_path.as_ptr(), &mut engine) };
        assert_eq!(rc, SK_ERR_OPEN_FAILED);
        assert!(engine.is_null());
    }

    #[test]
    fn free_null_is_noop() {
        unsafe { sk_free_engine(std::ptr::null_mut()) };
    }
}
