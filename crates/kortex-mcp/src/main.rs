#![forbid(unsafe_code)]
//! `kortex-mcp` — MCP server entrypoint.
//!
//! Usage: `kortex-mcp --corpus <path.json>`
//!
//! Reads newline-delimited JSON-RPC 2.0 messages from stdin, dispatches them
//! to [`kortex_mcp::Server::handle`], and writes any response as a single
...
use kortex_mcp::{parse_line, Server};

fn main() -> Result<()> {
    let corpus_path = parse_args()?;

    eprintln!("kortex-mcp: loading corpus from {}", corpus_path);
    ...
    eprintln!(
        "kortex-mcp: indexed {} entries; ready on stdio",
        corpus.entries.len()
    );
...
fn parse_args() -> Result<String> {
    ...
    anyhow::bail!("usage: kortex-mcp --corpus <path.json>")
}
