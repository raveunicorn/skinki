...
impl RaBitQ {
    ...
    /// self-describing — the Stage 6 FFI `sk_open` uses this to open the
    ...
    /// format the Stage 6 FFI `sk_open` will consume.
    ...
    pub fn load(dir: &std::path::Path) -> std::io::Result<Self> {
        ...
        if buf.len() < 37 || &buf[0..8] != RABITQ_MAGIC {
            return Err(bad("not a skinki rabitq index"));
        }
        let bits = buf[8];
...
mod tests {
    ...
    fn save_load_roundtrip_is_exact() {
        ...
        let dir = std::env::temp_dir().join(format!("skinki_rabitq_io_{}", std::process::id()));
