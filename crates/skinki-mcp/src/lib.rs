#![forbid(unsafe_code)]
//! Stage 6 — `skinki-mcp`: a Model Context Protocol (MCP) server exposing
//! skinki's graph memory (search + budgeted, cited context assembly) to MCP
...
//! `assemble_context` against a skinki corpus and get back dense, dated,
...
impl Server {
    ...
    /// `assemble_context`: call `skinki_graph::assemble_context` and render
...
mod tests {
    ...
    fn initialize_returns_server_info() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], json!("skinki-memory"));
