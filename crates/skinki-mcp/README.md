...
A Model Context Protocol (MCP) server that exposes skinki's graph memory —
multi-hop `search` and budgeted, cited `assemble_context` over a corpus
indexed with `skinki_graph::RelationRetriever` — to any MCP host (Claude
...
`skinki generate`) at startup and serves it for the process lifetime.
...
```bash
cargo run --release -p skinki-harness -- generate --years 1 --entries-per-day 2 --out /tmp/corpus.json
cargo build --release -p skinki-mcp
```
