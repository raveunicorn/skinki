...
| 1B | Static-distilled embedder + production retrieval path | **Closed — hypothesis falsified with data** (D1 + addendum): static tables cap at recall@10 ≈ 0.12 vs BM25 0.134 on the 594k-entry pooled row at ANY size (30 MB self-distilled 0.090, potion-8M 0.116, 130 MB potion-retr-32M 0.086). Deliverables kept: `SKEMB001` + `StaticEmbedder`, shared WordPiece tokenizer, `EmbedderSpec` seam, **T8 `RrfFusion`** (depth = k top-k consensus — recall 0.145, first config over BM25; instrument, not default) |
| 1C-B | **Pure-Rust sentence encoder** (`skinki-encoder`) — human-decided B-first over the sidecar (1C-A on hold as fallback) | **Closed — machinery proven, bars falsified-by-trend for bge-small** (D2 trend-close 2026-07-05, full-row rerun waived by human). Encoder itself is parity-perfect (per-layer ≤ 4.1e-6, e2e min cosine 1.0000000, byte-deterministic, zero deps). Trend row (41q/201k verified prefix): encoder 0.362 vs BM25 0.341, **`rrf(bm25+real)` 0.411** — fusion beats both parents, but bars ≥ 0.22/≥ 0.30 need 1.64×/2.24× over BM25 and 33M params deliver 1.06×/1.21×. Key finding: **BGE query-instruction prefix** (+25% recall; queries only, backfills unaffected). Sidecar fallback **rejected as product shape** (engine = self-contained binary; LLM = external swappable interface). Kept: `skinki-encoder`, `SKENC001`+converter+goldens, `encoder-embed`, `rrf(bm25+real)` instrument. See D2 verdict block in `specs/STAGE_1C_B_PURE_RUST_ENCODER.md` §6 |
| 1D | **Retrieval quality: mid-size multilingual encoder + int8 + space bridge** (`specs/STAGE_1D_RETRIEVAL_QUALITY.md`, draft) | **Next.** Three killable moves: M1 multilingual-e5-small (same compute as bge-small, multilingual, MIT) → escalation to base-class; M2 closed-form linear bridge (small tower embeds queries into big tower's space — least squares, no GPU); M3 int8 (human-approved escalation, safe Rust only). K0 = Unigram tokenizer parity. Bar: `rrf` ≥ 0.30 on D1 row; iteration on the cheap 41q/201k trend row |
