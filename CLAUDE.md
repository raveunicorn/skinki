...
| 1C-B | **Pure-Rust sentence encoder** (`skinki-encoder`) — human-decided B-first over the sidecar (1C-A on hold as fallback) | **In progress.** T0 GEMM kill-switch PASS (46.94 GF/s 4T sustained, M1 Max); D1 GO; T1 `SKENC001` + converter + goldens done; T2 forward pass done — **teacher parity: per-layer ≤ 4.1e-6, e2e min cosine 1.0000000**; zero deps, no `libm`, byte-deterministic across runs/threads/arch. Open: T3 (batch/backfill + query-latency pass: p95 67 ms vs 50 bar), T4 (`EmbedderSpec::Encoder` + emb artifact log), **D2 quality verdict** (bars ≥ 0.22 / ≥ 0.30 RRF vs semantic-real 0.438 reference) |
