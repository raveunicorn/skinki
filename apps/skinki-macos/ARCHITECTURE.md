# Skinki (macOS app) — Architecture

> **Status: parked (Stage 7 consumer wrapper).** After the Exocortex pivot, the
> primary artifact is the headless Rust engine in [`kortex/`](../../kortex/) —
> see the repo-root [`ARCHITECTURE.md`](../../ARCHITECTURE.md) and
> [`ROADMAP.md`](../../ROADMAP.md). This document describes the macOS consumer
> app that will wrap the engine at **Stage 7**. The SwiftUI/Tuist scaffolding
> below is intact and re-targeted to consume `kortex` (via Swift bindings)
> instead of the originally-planned in-app SQLite RAG.

This document describes the high-level architecture of the Skinki macOS app: how SwiftUI (UI + animations), on-device inference, and low-level macOS system APIs are composed into a single, fast, native, non-blocking application that surfaces the Exocortex engine's memory and insights.

- **Audience:** contributors.
- **Status:** parked scaffolding; revived at Stage 7 of the [Exocortex roadmap](../../ROADMAP.md).
