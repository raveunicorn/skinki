...
| ✅ **T8** (measure-first) hybrid RRF fusion: deterministic reciprocal-rank fusion of BM25 + `semantic-static` (potion-base-8M artifact), RRF k=60, tie-break by id — ~50 lines of pure Rust (`skinki_baseline::RrfFusion`), no new deps. **Done — measured 2026-07-04.** Depth sweep on the D1 row ({10,20,50,100} → recall@10 {**0.145**, 0.128, 0.122, 0.124}): fusing beyond top-k dilutes BM25's top ranks with static noise, so the adopted config is **depth = k (pure top-k consensus — no free parameter)**. At depth=k: recall@10 **0.145 (+8% vs BM25 0.134 — first config to beat the yardstick on this row)**, ndcg@10 **0.103** (+12% vs 0.092), answer@10 0.322 (−7% vs 0.347). **NOT adopted as served default**: +0.011 recall over 121 queries is within noise risk, answer@10 regresses, and serving would require the non-shipped 30 MB artifact. Recorded as the fusion pattern to re-apply the day a *discriminative* base lands (sidecar embeddings + BM25 is the expected payoff). `hybrid-rrf` column ships in `longmemeval-eval --pooled` as a permanent instrument | impl | cheaper | measured on the D1 row; recorded here; candidate served default if > BM25 0.134 (default flip is a separate decision — not taken, see verdict in the row) — **done** |
